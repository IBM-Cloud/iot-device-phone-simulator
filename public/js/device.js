...
(function(window) {
  ...
  function filterOffset(sample, channel) {
    ...
    var shiftedFCL =
      shifted_filter[channel] + (sample - last_sample[channel]) * 256;
  ...
  window.ondevicemotion = function(event) {
    ax = parseFloat(
      event.acceleration.x ||
        filterOffset(event.accelerationIncludingGravity.x, "ax") ||
        0
    );
    ay = parseFloat(
      event.acceleration.y ||
        filterOffset(event.accelerationIncludingGravity.y, "ay") ||
        0
    );
    az = parseFloat(
      event.acceleration.z ||
        filterOffset(event.accelerationIncludingGravity.z, "az") ||
        0
    );
  ...
  function onConnectFailure() {
    ...
    document.getElementById("status_message").innerHTML =
      "Could not connect to IBM Watson IoT Platform! Trying again in one second.";
    console.log(
      "Could not connect to IBM Watson IoT Platform! Trying again in one second."
    );
    setTimeout(connectDevice(), 1000);
  ...
  $("#connect").on("click", function() {
    ...
    window.iot_clientid =
      "d:" + org + ":" + window.devicetype + ":" + window.deviceId;
    window.client = new Paho.MQTT.Client(
      window.iot_host,
      window.iot_port,
      window.iot_clientid
    );
